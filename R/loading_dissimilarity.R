...
#'
#'\dontrun{
#' # Compare the loading pattern architecture of PC1 between two groups
#' loading_dissimilarity(
#'   obj = staRoxy_object,
#'   group1 = "LPS",
#'   group2 = "Control",
#'   pc_index = 1
...
loading_dissimilarity <- function(obj,
  ...
  # Helper to extract and normalize loadings
  extract_l <- function(p, i) {
    ld <- data.frame(Feature = rownames(p$rotation), Weight = p$rotation[, i])
  ...
  ggplot2::ggplot(df_c, ggplot2::aes(x = .data[[group1]], y = .data[[group2]])) +
    ...
    ggrepel::geom_text_repel(
      ggplot2::aes(label = ifelse(abs(.data[[group1]] - .data[[group2]]) >= threshold, Feature, "")),
